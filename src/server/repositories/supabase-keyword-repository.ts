...
export class SupabaseKeywordRepository implements KeywordRepository {
  ...
  async findManyByUserId(userId: string): Promise<KeywordRecord[]> {
    ...
    const analyses = await this.selectKeywordAnalyses(keywordIds)
    ...
    if (analysisIds.length > 0) {
      const { data: overlays, error: overlaysError } = await this.client
  ...
  async findById(
    ...
    const analyses = await this.selectKeywordAnalysesByKeywordId(keywordId)
    const analysisIds = analyses.map(item => item.id)
    const overlays: KeywordAnalysisOverlay[] = []

    if (analysisIds.length > 0) {
      const { data: overlayRows, error: overlayError } = await this.client
        .from('keyword_stock_overlays')
        .select(
          'analysis_id, id, ticker, company_name, display_order, created_at, last_refreshed_at, overlay_chart_timeseries(date, normalized_price, raw_price)'
        )
    ...
    return toKeywordRecord(
      ...
      toAnalysisSummaries(analyses, overlaysByAnalysisId)
    )
  }
  ...
  async reorder(userId: string, orderedIds: string[]): Promise<void> {
    ...
    const { data: ownedKeywords, error: ownedError } = await this.client
      .from('keywords')
      .select('id')
      .eq('user_id', userId)
      .in('id', orderedIds)
  ...
  private async selectKeywordAnalyses(
    keywordIds: string[]
  ...
  private async selectKeywordAnalysesByKeywordId(
    keywordId: string
  ): Promise<KeywordAnalysisRefreshRow[]> {
    const selectWithOrder =
      'id, keyword_id, region, period, search_type, trends_data, display_order, created_at, updated_at'
...
function toAnalysisSummaries(
  analyses: KeywordAnalysisRefreshRow[],
  overlaysByAnalysisId: Map<string, KeywordAnalysisOverlay[]>
): KeywordAnalysisSummary[] {
  return analyses
