...
export interface KeywordAnalysisSummary {
  ...
  overlays?: KeywordAnalysisOverlay[]
...
export interface KeywordStockOverlay {
  id: string // UUID
  analysis_id?: string // keyword_analysis.id 참조
  ticker: string // 종목 심볼 (예: AAPL)
  company_name: string // 회사명 (예: Apple Inc.)
  display_order: number // 그래프에서 표시할 순서
  created_at: string // ISO 8601 타임스탐프
  last_refreshed_at?: string | null // 마지막 주가 데이터 갱신 시각
  lastRefreshedAt?: string | null // 클라이언트 호환용 마지막 갱신 시각
  chart_data?: Array<{
    date: string
