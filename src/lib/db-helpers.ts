...
export function getSearchRecord(searchId: string): SearchRecord | null {
  ...
  const raw = db
    ...
    .get(searchId) as SearchRecordRaw | undefined
  if (!raw) return null
...
export function getSearchRecordByTicker(ticker: string): SearchRecord | null {
  const db = getDatabase()
  const raw = db
    .prepare('SELECT * FROM searches WHERE ticker = ?')
    .get(ticker) as SearchRecordRaw | undefined
  if (!raw) return null
  return parseSearchRecordRaw(raw)
}
...
export function savePriceData(
  ...
  withTransaction(db => {
    ...
    for (const point of priceData) {
      const id = `${searchId}-price-${point.date}`
      insertStmt.run(
        id,
        searchId,
        point.date,
...
export function getPriceData(searchId: string): PriceDataPoint[] {
  ...
  const rows = db
    .prepare(
      'SELECT date, close, open, high, low, volume FROM price_data WHERE search_id = ? ORDER BY date'
    )
 ...
 */
export function saveTrendsData(
  searchId: string,
  ...
  withTransaction(db => {
    ...
    for (const point of trendsData) {
      const id = `${searchId}-trends-${point.date}`
...
export function getTrendsData(searchId: string): TrendsDataPoint[] {
  ...
  const rows = db
    .prepare(
      'SELECT date, value FROM trends_data WHERE search_id = ? ORDER BY date'
    )
    .all(searchId) as Array<{
    date: string
    value: number
...
export function getDatabaseStats(): {
  ...
  const totalSearches = (
    db.prepare('SELECT COUNT(*) as count FROM searches').get() as {
      count: number
    }
  ).count
  const totalPriceDataPoints = (
    db.prepare('SELECT COUNT(*) as count FROM price_data').get() as {
      count: number
    }
  ).count
  const totalTrendsDataPoints = (
    db.prepare('SELECT COUNT(*) as count FROM trends_data').get() as {
      count: number
    }
  ).count
